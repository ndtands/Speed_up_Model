...
class TensorrtBase:
    ...
    def __init__(self, engine_file_path, *, gpu_id=0, dynamic_factor=1) -> None:
        cuda.init()
        # Create CUDA context
    ...
    def build_engine(cls,
        ...
        with open(onnx_file_path, 'rb') as model:
            if not parser.parse(model.read()):
                ...
                return None
        logger.info("✅ Completed parsing ONNX file")

        # default = 1 for fixed batch size
        builder.max_batch_size = 1
        
        if len(dynamic_shapes) > 0:
            logger.info(f"===> using dynamic shapes: {str(dynamic_shapes)}")
        ...
        if os.path.isfile(engine_file_path):
            try:
                ...
                logger.error(f"Cannot remove existing file: {engine_file_path}")
        ...
        if engine:
            ...
            logger.info(f"===> Serialized Engine Saved at: {engine_file_path}")
        else:
            logger.error("❌ build engine error")
        return engine
